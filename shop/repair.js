...
function renderServices() {
    ...
    grid.innerHTML = filtered.map(service => `
        <div class="service-card" data-service-id="${escapeAttr(service.id)}" data-title="${escapeAttr(service.title)}" data-price="${service.price || 0}">
    ...
    // Attach click handlers via JS (avoids breaking on quotes/apostrophes in titles)
    grid.querySelectorAll('.service-card').forEach(card => {
        card.addEventListener('click', () => {
            openBooking(card.dataset.serviceId, card.dataset.title, Number(card.dataset.price));
        });
    });
}
...
function renderParts() {
        ...
        return `
            <div class="spare-card">
                ...
                <div class="spare-info">
                    ...
                    ${stock > 0 ? 
                        `<button class="add-to-cart-btn" data-part-id="${escapeAttr(part.id)}" data-name="${escapeAttr(part.name)}" data-price="${part.price || 0}" data-image="${escapeAttr(img)}">
                            🛒 Add to Cart
                        </button>` :
    ...
    // Attach add-to-cart handlers via JS (avoids breaking on quotes/apostrophes in names)
    grid.querySelectorAll('.add-to-cart-btn[data-part-id]').forEach(btn => {
        btn.addEventListener('click', e => {
            e.stopPropagation();
            addToCart({
                id: btn.dataset.partId,
...
// Escapes a value for safe use inside an HTML attribute (handles quotes too)
