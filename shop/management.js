...
let orders = [];
let products = [];const $ = (s, p=document) => p.querySelector(s);
const $$ = (s, p=document) => [...p.querySelectorAll(s)];
let manager = null;
let offlineManager = false;
let orders = [];
...
async function saveBadge(productId, badge, was) {
  ...
  try {
    ...
    try { const c = await caches.open('sm-dynamics-cache-v3'); await c.delete('/api/products'); } catch (_) {}
    await loadAdminData();
  } catch (err) {
...
$('#productForm')?.addEventListener('submit', async e => {
  ...
  try {
    ...
    // Bust the service-worker cache for the products API so the management
    // page AND the main shop page both see the new product immediately.
    try {
      const cache = await caches.open('sm-dynamics-cache-v3');
      await cache.delete('/api/products');
    } catch (_) { /* cache API unavailable — safe to ignore */ }
